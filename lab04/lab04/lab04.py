def divide(quotients, divisors):
    ...
    """
    return {d:[x for x in divisors if x%d==0]  for d in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    a_x=get_lat(city_a)-lat
    a_y=get_lon(city_a)-lon
    b_x=get_lat(city_b)-lat
    b_y=get_lon(city_b)-lon
    return get_name(city_a) if a_x*a_x+a_y*a_y<b_x*b_x+b_y*b_y else get_name(city_b)
