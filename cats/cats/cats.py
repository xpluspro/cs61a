...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def check_keywords(paragraph):
        for word in split(paragraph):
            if lower(remove_punctuation(word)) in keywords:
                return True
        return False
    return check_keywords
    #return lambda paragraph: any([lower(word) in keywords for word in split(paragraph)])
    # END PROBLEM 2
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        if (typed, source, limit) not in cache:
            cache[(typed, source, limit)] = diff_function(typed, source, limit)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    minn=limit+1
    for i in word_list:
        if diff_function(typed_word, i, limit) < minn:
            mini=i
            minn=diff_function(typed_word, i, limit)
    if minn<=limit:
        return mini
    return typed_word
    # END PROBLEM 5
autocorrect = memo(autocorrect)
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit==0:
        return limit+1
    if typed=='': return min(len(source),limit+1)
    if source=='': return min(len(typed),limit+1)
    if typed[0]==source[0]:
        return minimum_mewtations(typed[1:],source[1:],limit)
    else:   
        add = minimum_mewtations(typed,source[1:],limit-1) # Fill in these lines
        remove = minimum_mewtations(typed[1:],source,limit-1) # Fill in these lines
        substitute = minimum_mewtations(typed[1:],source[1:],limit-1) # Fill in these lines
        ans=min(add,remove,substitute)
        return ans+1
        # BEGIN
        "*** YOUR CODE HERE ***"
        # END
...
minimum_mewtations = memo_diff(count(minimum_mewtations))
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(min(len(typed),len(source))):
        if typed[i]==source[i]: count+=1
        else:break
    print("ID:", user_id, "Progress:", count/len(source))
