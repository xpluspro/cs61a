...
def deep_map(f, s):
    ...
    for x in range(len(s)):
        if (type(s[x])==list) :deep_map(f,s[x])
        else: s[x]=f(s[x])
...
def balanced(m):
    ...
    if (is_mobile(end(left(m)))):
        ans=ans and balanced(end(left(m)))
    if (is_mobile(end(right(m)))):
        ans=ans and balanced(end(right(m)))
    ans=ans and (total_mass(end(left(m)))*length(left(m))==total_mass(end(right(m)))*length(right(m)))
    return ans
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if (label(t)=='berry'): ans=True
    else:ans= False
    if (is_leaf(t)):
        return ans
    for x in branches(t):
        ans=ans or berry_finder(x)
    return ans
